...
int main() {
	...
	list_insert(1, my_list->head, my_list);	
	list_insert(4, my_list->head, my_list);	
	
	struct node* my_node = list_find_data(my_list, 4); 
	int node_data = list_return_node_data(my_node); 
	list_insert(7, my_node, my_list);	
	printf("Node Data: %d\n", my_node->next->num);
